                ...
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody id="tabelPeminjamanBody">
                        ...
                        <tr>
                            ...
                            <td>{{ $p->prodi }}</td>
                            <td><span class="badge bg-info text-dark">{{ $p->item_pinjam }}</span></td>
                            <td>{{ $p->waktu_pinjam }}</td>
                            <td class="text-center">
                                <button class="btn btn-warning btn-sm text-white shadow-sm" data-bs-toggle="modal" data-bs-target="#EP1-{{ $p->id }}">
                                    <i class="fas fa-edit"></i> Edit
                                </button>
                                <form action="{{ route('kalender.destroy', $p->id) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                        ...
                        <div class="modal fade" id="EP1-{{ $p->id }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content border-0 shadow">
                                    <form action="{{ route('kalender.update', $p->id) }}" method="POST">
                                        @csrf @method('PUT')
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="prodi" class="form-control" value="{{ $p->prodi }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">Alat/Lab Dipinjam</label>
                                                <input type="text" name="item_pinjam" class="form-control" value="{{ $p->item_pinjam }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">Waktu Peminjaman</label>
                                                <input type="text" name="waktu_pinjam" class="form-control" value="{{ $p->waktu_pinjam }}" required>
                                            </div>
                                        </div>
